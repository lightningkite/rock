...
fun ViewContext.listTileGroup(setup: NView.() -> Unit) {
    column {
        space() in nativeBackground(BORDER_COLOR) in fullWidth() in sizedBox(SizeConstraints(minHeight = 1.px))
        setup()
        space() in nativeBackground(BORDER_COLOR) in fullWidth() in sizedBox(SizeConstraints(minHeight = 1.px))
    }
}
...
@ViewDsl
fun ViewContext.listTile(onClick: () -> Unit, setup: NView.() -> Unit) {
    row {
    ...
    } in padding(16.px) in interactive(
